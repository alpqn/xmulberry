...
Mulberry::Mulberry( QWidget* parent ): QMainWindow { parent }, ui { new Ui::Mulberry }
{
    ui->setupUi( this );
    ...
    connect( ui->selectWallpaperPushButton, &QPushButton::clicked, this, [&] {
    ...
    connect( ui->appComboBox, &QComboBox::currentTextChanged, this, [&] {
        ...
        ui->defaultMimeTypesList->clear();
        ui->defaultMimeTypesList->addItems( m_mimeApps[ui->appComboBox->currentIndex()].getActiveMimeTypes() );
    } );
    connect( ui->setAsDefaultPushButton, &QPushButton::clicked, this, [&] {
        ...
        m_mimeApps[ui->appComboBox->currentIndex()].setMimeTypesAdded(
            ui->mimeTypeComboBox->currentText().split( '|' ).first().trimmed() );
    } );
    populateFields();
    updateUI();
}
...
void Mulberry::loadSettings()
{
    ...
    dpmsValues.setSetter( [=, this] {
        ...
        ui->dpmsOff->setValue( list[2].toInt() );
    } );
    ...
    for (const auto& device: xinputOutput.filter( QRegularExpression( "^⎜.*↳.*" ) )) {
        ...
        if (QStringList line { listPropsOutput.filter( "libinput Accel Speed (" ) }; !line.isEmpty()) {
            ...
            sensitivityValues[name] = line[0].split( ':' ).last().trimmed();
        }

        if (QStringList line { listPropsOutput.filter( "libinput Natural Scrolling Enabled (" ) }; !line.isEmpty()) {
            ui->rScrollingComboBox->addItem( name );
            rScrollingValues[name] = line[0].split( ':' ).last().trimmed();
        }

        if (QStringList line { listPropsOutput.filter( "libinput Left Handed Enabled (" ) }; !line.isEmpty()) {
            ui->lefthandedComboBox->addItem( name );
            lefthandedValues[name] = line[0].split( ':' ).last().trimmed();
        }
    }

    Setting lefthanded { "Left-Handed Enabled", lefthandedValues,
                         "xinput set-prop pointer:'%1' 'libinput Left Handed Enabled' %2" };
    ...
    Setting rScrolling { "Reverse Scrolling Enabled", rScrollingValues,
                         "xinput set-prop pointer:'%1' 'libinput Natural Scrolling Enabled' %2" };
    ...
    Setting sensitivity { "Mouse Sensitivity", sensitivityValues,
...
void Mulberry::saveChanges()
{
    ...
    for (const auto& setting: m_settings) {
        if (setting.isChanged()) {
            ...
            ( currentValue.last() == "+" || currentValue.last() == "1" )
                ? currentValue.last() = "on"
                : ( ( currentValue.last() == "-" || currentValue.last() == "0" ) ? currentValue.last() = "off" : "" );
    ...
    if (resVBox->isEmpty()) {
        ui->statusbar->showMessage( "No changes were made to the configuration", 5000 );
    } else {
        ...
        connect( msg, &QDialog::accepted, this, [&] {
            ...
            if (sourceChangesCheckBox->isChecked()) {
                ...
                if (!xinitrcContent.contains(
                    ...
                    if (!xinitrc.open( QIODevice::WriteOnly )) {
                        std::cerr << "Cannot open xinitrc, check its file permissions\n";
                        return;
                    }
...
void Mulberry::writeChanges( const QList<Setting>& settingsChanged )
{
    ...
    if (!file.open( QIODevice::ReadWrite )) {
        std::cerr << "Cannot open " << filepath.absoluteFilePath().toStdString();
        exit( 1 );
    }
    ...
    for (const auto& setting: settingsChanged) {
        ...
        if (mulberryContent.contains( QRegularExpression( outCmd ) )) {
            mulberryContent.replace( QRegularExpression( outCmd ), setting.outputCmdFormatted() );
        } else {
            mulberryContent.append( setting.outputCmdFormatted() );
        }
    }
    if (!file.open( QIODevice::WriteOnly )) {
        std::cerr << "Cannot write changes to " << filepath.absoluteFilePath().toStdString();
        exit( 1 );
    }
    ...
    if (!configFile.open( QIODevice::ReadWrite )) {
        std::cerr << "Cannot open " << configFile.fileName().toStdString() << " check its permissions.\n";
        exit( 1 );
    }
    ...
    { // Put configSettings in a block, so that it doesn't get associated with the file after its closed
        ...
        for (const auto& app: m_mimeApps) {
            ...
            for (const auto& typeChanged: app.getMimeTypesAdded()) {
                configSettings.setValue( typeChanged, app.getFilename() );
            }
        }
